		...
		new_fun : function(pointer, img_ids) {
			...
			pointer.get_image_vals(img_ids).then(function(res) {
				...
				for (var k = 0; k < res.length; k++) {
					console.log(flag)
					if (flag == 0) {
						img_tag = img_tag + '<div><div class="col-xs-2"><a href="#1"><img id = ' + k + ' class = "img-fluid img_objs" src = "' + res[k] + '"></a></div></div></br>';
						flag = 1;
					} else {
						img_tag = img_tag + '<div><div class="col-xs-2"><a href="#1"><img id = ' + k + ' class = "img-fluid img_objs" src = "' + res[k] + '" ></a></div></div></br>';
						console.log("img_tagimg_tagimg_tag",img_tag)
					}

				}
			    console.log("IMG TAG",img_tag)
				setTimeout(function(){
					console.log(self.$('#my_div'));
